...
from commands.setup_cmd.claude_code_setup.claude_hooks import setHook, removeHook, _is_flow_command


def test_is_flow_command_with_flow_prefix():
    """Test _is_flow_command recognizes commands starting with 'flow'."""
    assert _is_flow_command("flow ping hello") is True
    assert _is_flow_command("flow prompt test") is True
    assert _is_flow_command("flow setup claude-code") is True


def test_is_flow_command_with_flow_cli_py():
    """Test _is_flow_command recognizes python commands with flow_cli.py."""
    assert _is_flow_command("python3 /path/to/flow_cli.py ping") is True
    assert _is_flow_command("/usr/bin/python3 flow_cli.py") is True


def test_is_flow_command_with_hook_script():
    """Test _is_flow_command recognizes flow hook scripts."""
    assert _is_flow_command("/path/to/flow_prompt_hook.py") is True
    assert _is_flow_command("/usr/local/bin/flow_prompt_hook.py") is True


def test_is_flow_command_with_non_flow_commands():
    """Test _is_flow_command rejects non-flow commands."""
    assert _is_flow_command("echo hello") is False
    assert _is_flow_command("git status") is False
    assert _is_flow_command("python3 script.py") is False
    assert _is_flow_command("/usr/bin/some_script.sh") is False
